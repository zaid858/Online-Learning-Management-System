  ...
  return (
    <div className="courses-page container">
      ...
      <header className="courses-header animate-fade-in">
        <span className="badge badge-primary">{t('Course Catalog')}</span>
        <h1>{t('Explore Our Professional Courses')}</h1>
      ...
      <main className="courses-grid-section">
        {filteredCourses.length > 0 ? (
              ...
              return (
                <article key={course.id} className="course-card animate-fade-in">
                  <div className="course-card-banner">
                    <span className="course-category-badge">{t(course.category)}</span>
                    <span className="course-difficulty-badge">{t(course.difficulty)}</span>
                    <div className="course-banner-overlay"></div>
                  </div>
                  
                  <div className="course-card-content">
                    <div className="course-instructor-meta">
                      <span>{t('By')} {t(getInstructorName(course.instructorId))}</span>
                    </div>

                    <h3 className="course-title">
                      <Link to={`/courses/${course.id}`}>{t(course.title)}</Link>
                    </h3>
                    
                    <p className="course-desc-short">{t(course.description)}</p>
                    
                    <div className="course-ratings">
                      <Star size={16} className="star-icon" fill="currentColor" />
                      <span className="rating-val">{course.rating}</span>
                      <span className="rating-count">({course.reviewsCount} {t('reviews')})</span>
                    </div>

                    <div className="course-meta-details">
                      <span className="meta-item">
                        <Clock size={15} /> {t(course.duration)}
                      </span>
                      <span className="meta-item">
                        <BookOpen size={15} /> {course.lecturesCount} {t('Lectures')}
                      </span>
                    </div>
                    ...
                    {isEnrolled ? (
                      ...
                      <div className="course-pricing-action">
                        <div className="course-price-wrap">
                          ...
                          <span className="price-value">{course.price.toLowerCase() === 'free' ? t('Free') : t(course.price)}</span>
                        </div>
                        <Link 
                          to={`/courses/${course.id}`} 
                          className="btn btn-secondary course-card-btn"
                        >
                          {t('View Details')} <ArrowRight size={15} />
                        </Link>
                      </div>
                    )}
                  </div>
          ...
          <div className="no-results-card">
            <h3>{t('No Courses Match')}</h3>
            <p>{t('Reset Filters Info')}</p>
            <button 
