...
export default function CourseViewer() {
  ...
  const [aiChatLog, setAiChatLog] = useState([
    { sender: 'ai', text: 'AI Chatbot Greeting' }
  ]);
  ...
  useEffect(() => {
    ...
    if (!enrolled.includes(foundCourse.id)) {
      alert(t('You must enroll in this course to view the classroom!'));
  ...
  if (!course || !activeLesson) {
    return <div className="container" style={{ padding: '8rem 2rem', textAlign: 'center' }}>{t('Loading classroom...')}</div>;
  }
  ...
  const handleSpeak = (rateValue = speechRate) => {
    ...
    const textToSpeak = `${t(activeLesson.title)}. ${t(activeLesson.content)}`;
    ...
    <div className="classroom-layout">
      ...
      <aside className="classroom-sidebar">
        <div className="sidebar-header-box">
          <Link to={`/courses/${course.id}`} className="class-back-btn">
            <ArrowLeft size={16} /> {t('Course Overview')}
          </Link>
          <h3 className="sidebar-course-title">{t(course.title)}</h3>
          
          {/* Progress Section */}
          <div className="sidebar-progress-zone">
            <div className="progress-details-text">
              <span>{t('Your Course Progress')}</span>
            ...
            <span className="fraction-lessons">
              {totalLessons.filter(l => completedLessonIds.includes(l.id)).length} {t('of')} {totalLessons.length} {t('completed')}
            </span>
          </div>
        </div>
        ...
        <div className="sidebar-playlist">
          {course.syllabus.map((chapter, chapterIndex) => (
            <div key={chapterIndex} className="playlist-chapter">
              <h4 className="playlist-chapter-title">{t(chapter.chapterTitle)}</h4>
                    ...
                    >
                      ...
                      >
                        ...
                        <div className="lesson-duration-row">
                          <PlayCircle size={12} /> {t(lesson.duration)}
                        </div>
                      </div>
                    </div>
                  );
      ...
      <main className="classroom-main">
          ...
          <section className="lesson-reader-container">
            <div className="text-viewport">
              <div className="reader-header">
                <span className="reader-tag">📖 {t('Interactive Text')}</span>
                <h2 className="reader-lesson-title">{t(activeLesson.title)}</h2>
              </div>
              
              <div className="reader-body">
                <p className="lesson-paragraph-text">{t(activeLesson.content)}</p>
              </div>
              ...
              <div className="speech-controls-bar">
                <div className="speech-btn-group">
                  {!isSpeaking ? (
                    <button onClick={() => handleSpeak()} className="btn btn-primary speech-btn" title={t('Listen to AI Reader')}>
                      <Volume2 size={16} /> {t('Listen Aloud')}
                    </button>
                  ) : (
                    <>
                      <button onClick={handlePauseResume} className="btn btn-secondary speech-btn">
                        {isPaused ? <Play size={14} /> : <Pause size={14} />} {isPaused ? t('Resume') : t('Pause')}
                      </button>
                      <button onClick={handleStop} className="btn btn-secondary speech-btn stop-btn">
                        <StopSquare size={14} fill="currentColor" /> {t('Stop')}
                      </button>
                    </>
                  )}
                </div>
                ...
                <div className="rate-selector-group">
                  <span className="rate-label">{t('AI Speed')}</span>
                ...
                {isSpeaking && !isPaused && (
                  <div className="speech-wave-indicator" title={t('AI reading in progress')}>
        ...
        <section className="classroom-tabs-nav">
          ...
          >
            <FileText size={16} /> {t('Dynamic Notes')}
          </button>
          <button 
            className={`tab-item-btn ${activeTab === 'qa' ? 'active' : ''}`}
            onClick={() => setActiveTab('qa')}
          >
            <MessageSquare size={16} /> {t('Discussion Forum')}
          </button>
          <button 
            className={`tab-item-btn ${activeTab === 'ai' ? 'active' : ''}`}
            onClick={() => setActiveTab('ai')}
          >
            <MessageSquare size={16} /> {t('AI Tutor Assistant')}
          </button>
          <button 
            className={`tab-item-btn ${activeTab === 'quiz' ? 'active' : ''}`}
            onClick={() => setActiveTab('quiz')}
          >
            <HelpCircle size={16} /> {t('Chapter Quizzes')}
          </button>
        </section>
        ...
        <section className="classroom-tab-panel-content">
          ...
          {activeTab === 'overview' && (
            <div className="tab-pane-fade-in">
              <h2>{t('About this Lecture')}</h2>
              <p style={{ marginBottom: '1.5rem', fontSize: '1.05rem', color: 'var(--text-main)' }}>
                {t('About Info').replace('the curriculum', `the **${t(course.title)}** curriculum`)}
              </p>
              <h3>{t('Topics Covered')}</h3>
              <ul className="topics-list" style={{ marginLeft: '1.5rem', marginTop: '0.75rem', gap: '0.5rem', display: 'flex', flexDirection: 'column' }}>
                <li>{t('Structural definitions and development environments.')}</li>
                <li>{t('Best practices for building fast, responsive operations.')}</li>
                <li>{t('Debugging common configuration discrepancies.')}</li>
                <li>{t('Testing local files for responsiveness.')}</li>
              </ul>
            </div>
          )}
          ...
          {activeTab === 'notes' && (
            <div className="tab-pane-fade-in notes-pane">
              <div className="notes-header-row">
                ...
                >
                  <Save size={16} /> {saveNotesSuccess ? t('Saved') : t('Save Notes')}
                </button>
              </div>
              <p className="tab-subtitle-text">{t('Lecture Notepad Info')}</p>
              <textarea 
                className="notes-textarea" 
                placeholder={t('Type Notes Place')} 
                value={studentNotes}
          ...
          {activeTab === 'qa' && (
            <div className="tab-pane-fade-in qa-pane">
              <h2>{t('Discussion Forum')}</h2>
              <p className="tab-subtitle-text">{t('Forum Info')}</p>
              ...
              <form onSubmit={handlePostQuestion} className="post-question-form">
                ...
                />
                <button type="submit" className="btn btn-primary">{t('Post Question')}</button>
              </form>
                ...
                {qaThreads.map((thread) => (
                  <div key={thread.id} className="qa-thread-item">
                    ...
                    {thread.answers.length > 0 ? (
                      ...
                      <div className="no-answers-prompt">{t('Empty Answers')}</div>
                    )}
                  </div>
                ))}
              </div>
          ...
          {activeTab === 'quiz' && (
            <div className="tab-pane-fade-in quiz-pane">
              <div className="quiz-header-row">
                <h2>{t('Chapter Quizzes')} ({activeQuizzes.length} {t('Questions')})</h2>
                {quizSubmitted && activeQuizzes.length > 0 && (
                  <div className="quiz-score-badge">
                    {t('Score')}: {quizScore} / {activeQuizzes.length} (
                    {Math.round((quizScore / activeQuizzes.length) * 100)}%)
                  </div>
                )}
              </div>
              <p className="tab-subtitle-text">{t('Quiz Info')}</p>
                  ...
                  {activeQuizzes.map((quiz, qIdx) => {
                    ...
                    return (
                      <div key={qIdx} className={`quiz-question-card ${quizSubmitted ? (isCorrect ? 'correct' : 'incorrect') : ''}`}>
                        <h4 className="quiz-question-title">Q{qIdx + 1}: {t(quiz.question)}</h4>
                        ...
                        {quizSubmitted && (
                          <div className={`quiz-explanation-box ${isCorrect ? 'correct' : 'incorrect'}`}>
                            <strong>{t('Explanation')}:</strong> {t(quiz.explanation)}
                          </div>
                        )}
                      </div>
                    );
            ...
            <div className="tab-pane-fade-in ai-chatbot-pane">
              ...
              <div className="ai-chat-messages">
                ...
                {aiIsTyping && (
                  <div className="ai-chat-bubble ai typing-indicator">
                    <span>{t('AI Tutor is formulating explanation...')}</span>
                  </div>
                )}
              </div>
              ...
              <form onSubmit={handleSendAiMessage} className="ai-chat-input-area">
                <input 
                  ...
                  placeholder={`${t('Type message')} "${t(activeLesson.title)}"...`}
                  value={aiInput}
