...
export default function InstructorWorkspace() {
  ...
  useEffect(() => {
    ...
    if (!loggedIn || role !== 'instructor') {
      alert(t("Access Denied. Instructor role required."));
      navigate('/');
      return;
    }
  ...
  const handleAddLesson = (e) => {
    ...
    if (!selectedCourseId || !lessonTitle.trim() || !lessonContent.trim() || !lessonDuration.trim()) {
      alert(t("Please fill in all inputs."));
      return;
    }
  ...
  const handleAddQuiz = (e) => {
    ...
    if (!quizCourseId || !quizQuestion.trim() || quizOptions.some(o => !o.trim()) || !quizExplanation.trim()) {
      alert(t("Please fill in all quiz inputs."));
      return;
    }
  ...
  return (
    <div className="instructor-page container">
      ...
      <header className="inst-header animate-fade-in">
        <span className="badge badge-primary">{t('Educator Desk')}</span>
        <h1>{t('Instructor Workspace')}</h1>
        <p className="subtitle">{t('Instructor Workspace Subtitle')}</p>
      </header>
      ...
      <main className="inst-panel-body animate-fade-in">
        ...
        {activeTab === 'analytics' && (
          <div className="tab-pane-wrapper">
            <h2>{t('Your Class Statistics')}</h2>
            <p className="section-desc-sub text-muted">{t('Statistics Description')}</p>

            <div className="inst-analytics-grid">
              <div className="inst-stat-card">
                <Users size={28} className="stat-icon-theme" />
                <strong>1,240</strong>
                <span>{t('Total Enrolled')}</span>
              </div>
              <div className="inst-stat-card">
            ...
            <h3 style={{ marginTop: '2.5rem', marginBottom: '1rem' }}>{t('Active Courses Directory')}</h3>
            <div className="inst-courses-list">
              {localCourses.map(course => (
                <div key={course.id} className="inst-course-row">
                  <div>
                    <h4>{course.title}</h4>
                    <span className="text-muted">{t(course.category)} • {t(course.difficulty)}</span>
                  </div>
                  <div className="inst-course-meta-node">
                    <strong>{course.syllabus.reduce((acc, ch) => acc + ch.lessons.length, 0)} {t('Lectures')}</strong>
                    <span>{course.quizzes?.length || 0} {t('Questions')}</span>
                  </div>
                </div>
              ))}
            </div>
        ...
        {activeTab === 'lessons' && (
          <div className="tab-pane-wrapper">
            <h2>{t('Append Lecture Lesson')}</h2>
            <p className="section-desc-sub text-muted">{t('Lesson Builder Description')}</p>

            {lessonSuccess && (
              <div className="custom-alert custom-alert-success animate-fade-in" style={{ marginBottom: '1.5rem' }}>
                <CheckCircle size={18} />
                <span>{t('Success! New lecture content successfully written to course curriculum.')}</span>
              </div>
            )}

            <form onSubmit={handleAddLesson} className="workspace-form">
              <div className="form-group">
                <label className="form-label" htmlFor="course-select">{t('Select Target Course')}</label>
                <select 
                  id="course-select"
              ...
              <div className="form-grid-2">
                <div className="form-group">
                  <label className="form-label" htmlFor="lesson-title">{t('Lecture Title')}</label>
                  <input 
                    id="lesson-title"
                    type="text" 
                    className="form-input" 
                    placeholder={t('e.g. 1.5 Introduction to Flexbox Layouts')}
                    value={lessonTitle}
                ...
                <div className="form-group">
                  <label className="form-label" htmlFor="lesson-duration">{t('Lecture Read Time')}</label>
                  <input 
                    id="lesson-duration"
                    type="text" 
                    className="form-input" 
                    placeholder={t('e.g. 15 mins')}
              ...
              <div className="form-group">
                <label className="form-label" htmlFor="lesson-content">{t('Lecture Content')}</label>
                <textarea 
                  id="lesson-content"
                  className="form-input inst-textarea" 
                  placeholder={t('Write the full textbook content of the lesson here for students to read and listen...')}
                  value={lessonContent}
              ...
              <button type="submit" className="btn btn-primary">{t('Add Lecture to Course')}</button>
            </form>
          </div>
        )}
        ...
        {activeTab === 'qa' && (
          <div className="tab-pane-wrapper">
            <h2>{t('Moderate Student Q&A Board')}</h2>
            <p className="section-desc-sub text-muted">{t('Moderate Board Description')}</p>

            {qaSuccess && (
              <div className="custom-alert custom-alert-success animate-fade-in" style={{ marginBottom: '1.5rem' }}>
                <CheckCircle size={18} />
                <span>{t('Instructor response published successfully! Sync complete.')}</span>
              </div>
            )}
            ...
            <div className="inst-qa-threads-list">
              {localQa.length > 0 ? (
                  ...
                  <div key={thread.id} className="inst-qa-thread-card">
                    <div className="inst-thread-question">
                      <div className="thread-user-meta">
                        <span>{t('Student')}: <strong>{thread.author}</strong></span>
                    ...
                    <div className="inst-reply-action-wrap">
                      <textarea
                        ...
                        placeholder={t('Write your official instructor reply here...')}
                ...
                <div className="profile-empty-state text-center">
                  ...
                  <h3>{t('Q&A Board Empty')}</h3>
                  <p>{t('Students have not posted any discussion questions yet.')}</p>
                </div>
              )}
            </div>
          </div>
        ...
        {activeTab === 'quizzes' && (
          <div className="tab-pane-wrapper">
            <h2>{t('Add Quiz Checkpoint Question')}</h2>
            <p className="section-desc-sub text-muted">{t('Quiz Builder Description')}</p>

            {quizSuccess && (
              <div className="custom-alert custom-alert-success animate-fade-in" style={{ marginBottom: '1.5rem' }}>
                <CheckCircle size={18} />
                <span>{t('Success! New quiz question appended. Students will see it in the chapter quiz tab.')}</span>
              </div>
            )}

            <form onSubmit={handleAddQuiz} className="workspace-form">
              <div className="form-group">
                <label className="form-label" htmlFor="quiz-course-select">{t('Select Course')}</label>
                <select 
              ...
              <div className="form-group">
                <label className="form-label" htmlFor="quiz-question">{t('Question Description')}</label>
                <input 
                  id="quiz-question"
                  type="text" 
                  className="form-input" 
                  placeholder={t('e.g. Which keyword declares a block-scoped local variable?')}
                  value={quizQuestion}
              ...
              <div className="form-grid-2">
                {quizOptions.map((opt, oIdx) => (
                  <div key={oIdx} className="form-group">
                    <label className="form-label" htmlFor={`quiz-option-${oIdx}`}>{t('Option')} {String.fromCharCode(65 + oIdx)}</label>
                    <input 
                      id={`quiz-option-${oIdx}`}
                      type="text" 
                      className="form-input" 
              ...
              <div className="form-grid-2">
                <div className="form-group">
                  <label className="form-label" htmlFor="correct-answer">{t('Correct Answer Option')}</label>
                  <select 
                    id="correct-answer"
                    className="form-input" 
                  ...
                  >
                    <option value={0}>{t('Option A')}</option>
                    <option value={1}>{t('Option B')}</option>
                    <option value={2}>{t('Option C')}</option>
                    <option value={3}>{t('Option D')}</option>
                  </select>
                </div>

                <div className="form-group">
                  <label className="form-label" htmlFor="quiz-explanation">{t('Explanation / Reasoning')}</label>
                  <input 
                    id="quiz-explanation"
                    type="text" 
                    className="form-input" 
                    placeholder={t('e.g. The let keyword declares block scoped local variables.')}
              ...
              <button type="submit" className="btn btn-primary">{t('Publish Quiz Question')}</button>
            </form>
          </div>
        )}
